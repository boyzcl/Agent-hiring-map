...
- 7,217 条**安全证据索引**；
- 全球公开来源有界恢复后的 1,315 条岗位记录：888 条已核实岗位、427 条高概率岗位；
- 745 条经过标题、稳定定位、日期、来源和公开访问门筛选的严格当前岗位；
...
- Other/Global/Unknown 共 4,643 条冻结线索全部获得唯一终态，其中 349 个去重后的官方岗位进入岗位列表，254 个额外通过严格 Current 门；
...
本次全球恢复承接此前中美存量结果，并对剩余 4,643 条 Other/Global/Unknown 线索做全量分层：349 个去重后的岗位进入岗位列表，其中 254 个通过严格 Current 门；其余线索保留为非岗位、信息不足、认证阻塞、来源变化或地域待复核等可审计终态。实际公开数量和差异原因见：
